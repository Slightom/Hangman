...
function start() {
    ...
    for (i = 0; i < 35; i++) {
        tresc += '<div class="litera" id="n' + i + '" onClick="sprawdz(' + i + ')">' + litery[i] + '</div>'
        if ((i + 1) % 7 == 0)
...
function sprawdz(nr) {
    ...
    //wygrana
    if (haslo == haslo1)
        document.getElementById("alfabet").innerHTML = "Yeeeah bitch! You are the winner! Podano prawidłowe haslo: " +
        haslo + '<br/><br/><span class="reset" onclick="location.reload()">JESZCZE RAZ?</span>';

    //przegrana
    if (ileSkuch == 9) {
        document.getElementById("alfabet").innerHTML = "Przegrałeś ziomuś: Poprawne hasło to: " +
            haslo + '<br/><br/><span class="reset" onclick="location.reload()">JESZCZE RAZ?</span>';
    }
}